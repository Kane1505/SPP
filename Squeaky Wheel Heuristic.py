...
# Import data
w_h = []
with open('Data sets/BENG10.TXT') as f:
    lines = f.readlines()
...
# Set up parameters
time_limit = 30        # Time after which the program terminates
iteration_limit = 100000  # Number of iterations after which the program terminates
...
while time.time() - t <= time_limit and iteration <= iteration_limit:
    ...
    x_y_w_h = []        # Intialize a list to store coordinates
    iteration += 1
    ...
    while len(w_h) != 0:    # While items are not packed
        ...
        if index_min == 0:  # If the gap is against the left wall
            ...
            if current_best == -1:   # If no rectangle fits, raise the gap to the height of the rectangle on the right
                ...
                x_y_w_h.append([len_gap - w_h[current_best_index][1],sheet[index_min],w_h[current_best_index][1],w_h[current_best_index][0]])   # Store coordinates of the rectangle that has been placed
                ...
                x_y_w_h.append([len_gap - w_h[current_best_index][0],sheet[index_min],w_h[current_best_index][0],w_h[current_best_index][1]]) # Store coordinates of the rectangle that has been placed
            ...
            if current_best == -1:   # If no rectangle fits, raise the gap to the left neighbour
                ...
                x_y_w_h.append([index_min,sheet[index_min],w_h[current_best_index][1],w_h[current_best_index][0]])  # Store the coordinates of the rectangle that has been placed
                ...
                x_y_w_h.append([index_min,sheet[index_min],w_h[current_best_index][0],w_h[current_best_index][1]])  # Store the coordinates of the rectangle that has been placed
            ...
            if tallest_neighbour == 1:  # If the tallest neighbour is on the left:
                ...
                if current_best == -1:   # If no rectangle fits, raise the gap to the shortest neighbour
                    ...
                    x_y_w_h.append([index_min,sheet[index_min],w_h[current_best_index][1],w_h[current_best_index][0]]) # Store coordinates of placed rectangle
                    # ax.add_patch(matplotlib.patches.Rectangle((index_min,sheet[index_min]),w_h[current_best_index][1],w_h[current_best_index][0],color = next(colors_cycle)))
                    # ax.add_patch(matplotlib.patches.Rectangle((index_min,sheet[index_min]),w_h[current_best_index][1],w_h[current_best_index][0], fill = None))                
                    for j in range(w_h[current_best_index][1]): # Update sheet height
                        sheet[index_min + j] += w_h[current_best_index][0]                
                    ...
                    x_y_w_h.append([index_min,sheet[index_min],w_h[current_best_index][0],w_h[current_best_index][1]]) # Store coordinates of placed rectangle
                    # ax.add_patch(matplotlib.patches.Rectangle((index_min,sheet[index_min]),w_h[current_best_index][0],w_h[current_best_index][1],color = next(colors_cycle)))
                    # ax.add_patch(matplotlib.patches.Rectangle((index_min,sheet[index_min]),w_h[current_best_index][0],w_h[current_best_index][1], fill = None))                
                    for j in range(w_h[current_best_index][0]): # Update sheet height
                        sheet[index_min + j] += w_h[current_best_index][1]                
                ...
                if current_best == -1:   # If no rectangle fits, raise the gap to the level of the shortest neighbour
                    ...
                    x_y_w_h.append([index_min + len_gap - w_h[current_best_index][1],sheet[index_min],w_h[current_best_index][1],w_h[current_best_index][0]]) # Store coordinates of placed rectangle
                    ...
                    x_y_w_h.append([index_min + len_gap - w_h[current_best_index][0],sheet[index_min],w_h[current_best_index][0],w_h[current_best_index][1]]) # Store coordinates of placed rectangle
    ...
    while opt_finish == 0:
        ...
        for i in range(x_y_w_h[index_max][2]):  # Remove the rectangle and update the sheet accordingly
            sheet_new[x_y_w_h[index_max][0] + i] -= x_y_w_h[index_max][3]
        fit = 0
        if x_y_w_h[index_max][2] > x_y_w_h[index_max][3]: # If the width is larger than the height of the removed rectangle, optimization is finished
            opt_finish = 1
        else:
            width = x_y_w_h_new[index_max][3]   # Rotate the rectangle
            ...
            while fit == 0:
                ...
                if index_min == 0:  # If the lowest gap is against the left wall:
                    if width <= len_gap:    # Check if the rectangle fits
                        x_y_w_h_new[index_max][0] = index_min + len_gap - width     # Store coordinates of the rectangle
                    ...
                    if width <= len_gap:    # Check if the rectangle fits
                        x_y_w_h_new[index_max][0] = index_min   # Store coordinates of the rectangle
                    ...
                    if tallest_neighbour == 1:  # If the tallest neighbour is to the right
                        if width <= len_gap:    # Check if the rectangle fits in the gap
                            x_y_w_h_new[index_max][0] = index_min + len_gap - width # Store the coordinates of the rectangle
                            x_y_w_h_new[index_max][1] = sheet_new[index_min + len_gap - width]
                        ...
                        if width <= len_gap:    # Check if the rectangle fits in the gap
                            x_y_w_h_new[index_max][0] = index_min   # Store the coordinates
                            x_y_w_h_new[index_max][1] = sheet_new[index_min]
                            ...
                            for i in range(len_gap):    
                                sheet_new[index_min + i] = sheet_new[index_min + len_gap]
    for i in range(n):  # Find the heights of all rectangles
        rec_height = x_y_w_h[i][1] + x_y_w_h[i][3]
        penalty_points[i] += max([0,rec_height - target_height])
        w_h.append([x_y_w_h[i][2],x_y_w_h[i][3],penalty_points[i]])
    if max_height <= max_height_tot:    
        max_height_tot = max_height
        x_y_w_h_best = copy.deepcopy(x_y_w_h)
